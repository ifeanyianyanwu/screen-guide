...
export default async function TvShowDetailsPage({
  ...
  const details = await getTvDetails<TVShowDetails>(Number(id));
  const credits = await getTvCredits<Credits>(Number(id));
  const trailer = await getTvTrailers<Trailers>(Number(id));
  const reviews = await getTvReviews<Reviews>(Number(id));
  const similar = await getSimilarShows<Media>(Number(id));
